...
ความพร้อมสำหรับ production คือ **checklist** ไม่ใช่การตัดสินใจครั้งเดียว remote backend ที่ lock ไว้ตั้งแต่วันแรก, provider กับ module version ที่ pin ไว้, ทุก `plan` ถูก review ใน CI หา replacement ที่ไม่คาดคิด, secret เก็บออกจาก variable และ state เท่าที่ทำได้พร้อม encrypt backend ไว้ตอน rest, `tflint`/scanner/policy-as-code ต่อเข้ากับ CI, Terragrunt **`dependency`** block ขับ rollout order แทนความจำของคน และ `terraform test` ครอบคลุม module ที่ไม่ใช่เรื่องธรรมดา รวมกันแล้วคือระบบเดียวที่เชื่อมกัน — shared backend แบบ **`root.hcl`**, หลาย unit ที่ต่อกันด้วย `dependency` block, deploy ผ่าน pipeline ที่รัน `terragrunt run --all plan` ทุก pull request และ `terragrunt run --all apply` หลัง merge เท่านั้น — ไม่ใช่กองข้อเท็จจริงที่แยกกันอยู่
</Callout>
...
export const quizProductionChecklist = [
  ...
  },
  {
    ...
    options: [
      ...
      'ทำให้ unit หนึ่ง reference output จริงที่คำนวณแล้วของอีก unit ได้ตรง ๆ และขับลำดับ apply อัตโนมัติ แทนที่จะพึ่งคนจำว่า vpc ต้อง apply ก่อน rds',
