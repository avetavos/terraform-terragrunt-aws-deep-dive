...
export const quizInclude = [
  {
    ...
    options: [
      ...
      'เดินขึ้น directory tree เพื่อหาไฟล์ที่ระบุชื่อไว้ แล้ว merge config นั้นเข้ากับ unit ปัจจุบัน ไม่ว่าจะอยู่ลึกแค่ไหน',
  ...
  },
  {
    ...
    options: [
      ...
      'parse Terragrunt config ไฟล์อื่น แล้ว expose เนื้อหา เช่น locals block ให้ unit ปัจจุบันใช้ ทำให้แชร์ค่าลงไปแค่บางส่วนของ tree แทนที่จะเป็นทั้งหมด',
  ...
  },
  {
    ...
    options: [
      'ใช้ต่อไม่ได้ ต้องแก้ include path เองทุกครั้งที่ย้าย',
      'ค้นหาไฟล์ที่ระบุชื่อไว้แบบ dynamic ขึ้นไปเรื่อย ๆ แทนที่จะพึ่ง relative path ที่ hard-code ไว้',
      'Terragrunt cache path เดิมไว้แล้วเพิกเฉยต่อการย้าย',
      'ค้นหาแค่ directory ปัจจุบันเท่านั้น การย้าย unit จึงไม่มีผลอะไรเลย',
    ],
    answer: 1,
  },
];
