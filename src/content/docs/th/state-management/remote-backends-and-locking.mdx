...
export const quizBackends = [
  ...
  },
  {
    ...
    options: [
      'เปิด versioning บน S3 bucket ให้อัตโนมัติ',
      'ให้ native state locking โดยใช้ conditional-write lock file ตรงใน S3 เอง ทำให้ไม่ต้องมี DynamoDB table แยกที่ argument dynamodb_table แบบเก่าต้องการ',
      'เข้ารหัส state file ที่ rest ซึ่ง DynamoDB locking ทำไม่ได้',
      'ไม่มีผลอะไรเลย DynamoDB locking ยังจำเป็นสำหรับความถูกต้องอยู่ดี',
    ],
    answer: 1,
  },
  {
    ...
    options: [
      'กันไม่ให้ใครรัน terraform plan ได้เลย',
      'กันไม่ให้ apply สองตัวเขียนทับ state file เดียวกันพร้อมกัน ซึ่งอาจทำให้ state corrupt หรือทำให้การเปลี่ยนแปลงหายไป',
      'กันไม่ให้ Terraform อ่าน state file ได้เลย',
      'กันไม่ให้ resource ถูกลบโดยไม่ตั้งใจ',
    ],
    answer: 1,
  },
];
