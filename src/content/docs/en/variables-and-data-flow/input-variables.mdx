...
## Two newer arguments: ephemeral and deprecated

Two more arguments are worth knowing because they solve problems the ones above cannot.

`sensitive = true` hides a value from output but still writes it to state. **`ephemeral = true`** (Terraform 1.10+) actually keeps it out: the value is available while the operation runs and is omitted from both state and the plan file.
...
The trade-off is that an ephemeral value can only flow into places that also refuse to persist it — a write-only resource argument, another ephemeral variable or resource, a provider block, or a provisioner. Assigning one to an ordinary resource argument is an error. The Secrets and Credential Management lesson shows the full pattern with a write-only argument on the receiving end.

**`deprecated`** (Terraform 1.15+) is for module authors renaming an input without breaking every caller at once. The string you supply is shown as a warning whenever someone sets that variable:
...
Callers keep working and get told what to change, which is what makes a staged migration possible in a repository where you do not control every consumer of the module.
