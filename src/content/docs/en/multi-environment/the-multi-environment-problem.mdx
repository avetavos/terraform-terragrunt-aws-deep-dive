...
On day one, all three files are identical except for a handful of hardcoded values — an instance size here, a bucket name there. It works, and it feels fast: no shared modules to design, no abstractions to get right, just three folders of ordinary `.tf` code.
...
export const quizMultiEnvProblem = [
  {
    ...
    options: [
      ...
      'Copy-pasting directories is not allowed by the AWS provider',
  ...
  },
  {
    ...
    options: [
      ...
      'AWS periodically changes resource defaults on its own',
  ...
  },
  {
    ...
    options: [
      ...
      'Limiting how far a mistake, bad apply, or leaked credential in one environment can actually reach, often by placing environments in separate AWS accounts or regions',
