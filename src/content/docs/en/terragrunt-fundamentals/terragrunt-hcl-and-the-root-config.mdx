...
A Terragrunt **unit** is one directory with its own `terragrunt.hcl` and its own state, wrapping one Terraform module; the shared parent config sits one level up and is now conventionally named **`root.hcl`** (not `terragrunt.hcl`) specifically to avoid confusing "the shared root file" with "this unit's own file" — older repos that still call it `terragrunt.hcl` continue to work. `root.hcl` declares a `remote_state` block with a `generate` sub-block so every unit gets an identical, centrally defined S3 **backend.tf** (and, via a top-level `generate "provider"` block, a **provider.tf**) instead of that boilerplate being hand-copied into every unit. Each unit's own `terragrunt.hcl` then only needs a `terraform { source = ... }` block pointing at the actual module it should run.
</Callout>
...
export const quizRootConfig = [
  {
    ...
    options: [
      'A single AWS resource such as one EC2 instance',
  ...
  },
  {
    ...
    options: [
      'It only validates that an S3 bucket exists without writing any file',
