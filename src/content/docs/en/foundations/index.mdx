...
description: 'A developer-focused, zero-to-hero course on Terraform and Terragrunt targeting AWS — HCL, state, variables and modules, multi-environment design, Terragrunt, and running infrastructure as code in production.'
sidebar:
  order: 1
---
...
Every command, HCL example, and CLI flag in this course was checked against **Terraform 1.16**, **Terragrunt 1.1**, and the **`hashicorp/aws` provider `~> 6.0`** — the current stable releases at the time of writing.
...
Terraform is **declarative infrastructure as code**: you describe desired AWS resources in HCL, and Terraform plans and applies the changes needed to get there, tracking everything in **state**. This course teaches Terraform and Terragrunt from a developer's seat — the workflow, the guarantees, and the production behavior — starting from HCL itself.
</Callout>
